...
exports.newPassword=async(req,res,next)=>{
    ...
    try{
        const user=await User.fineOne({whee:{resetToken:resetToken,resetTokenExpiration:{[Op.gt]:Date.now()}}})
...
exports.logout=async(req,res,next)=>{
    try{
        ...
        if(!err.statusCode){
            ...
            const userToken=await db.token.findOne({where:{token:token}})
            if(!userToken){
                return res.status(403).json({message:"User Not authenticated"})
            }
            await userToken.destroy()
            return res.status(200).json({message:"logou done"})
        }
        next(err);
    }
}